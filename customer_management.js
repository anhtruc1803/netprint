...
document.addEventListener('click', () => {
    const cm = document.getElementById('custContextMenu');
...
function renderCustomerTable() {
    ...
    tbody.innerHTML = paginated.map(c => {
        ...
        return `
        <tr data-id="${c.id}" onclick="editCustomerPage(${c.id})" oncontextmenu="showCustContextMenu(event, ${c.id})" style="cursor:pointer;">
...
window.showCustContextMenu = function (e, id) {
    ...
    let cm = document.getElementById('custContextMenu');
    if (!cm) {
        cm = document.createElement('div');
        cm.id = 'custContextMenu';
        cm.className = 'cust-context-menu';
        cm.innerHTML = `
            <div class="cust-cm-item" onclick="custCmEdit()"><span class="cust-cm-icon">✏️</span> Chỉnh sửa</div>
            <div class="cust-cm-divider"></div>
            <div class="cust-cm-item cust-cm-delete" onclick="custCmDelete()"><span class="cust-cm-icon">🗑️</span> Xóa khách hàng</div>
        `;
    ...
    let x = e.pageX;
    let y = e.pageY;
    if (x + cm.offsetWidth > window.innerWidth) x = window.innerWidth - cm.offsetWidth;
    if (y + cm.offsetHeight > window.innerHeight) y = window.innerHeight - cm.offsetHeight;
