...
// Tạo HTML cho 1 card lịch sử (1 ô duy nhất, gọn gàng)
function createHistoryCard(quote) {
    ...
    const title = `SP: ${spec} | ${paper} | ${print} | ${lam}${processing !== 'Không' ? ' | ' + processing : ''}`;
        ...
        <div class="history-card-single" data-id="${quote.id}">
            ...
            <div class="history-card-header">
                ...
                <div class="history-actions">
                    <button class="btn-icon" onclick="copyQuoteFromHistory(${quote.id})" title="Copy">
                        <span>📋</span>
                    </button>
                    <button class="btn-icon" onclick="editQuoteFromHistory(${quote.id})" title="Sửa">
                        <span>✏️</span>
                    </button>
            ...
            <div class="history-card-body-single">
                ...
                <span class="history-info">📏 ${spec}</span>
                <span class="history-separator">•</span>
                <span class="history-info">📄 ${paper}</span>
                <span class="history-separator">•</span>
                <span class="history-info">🖨️ ${print}</span>
                <span class="history-separator">•</span>
                <span class="history-info">✨ ${lam}</span>
                <span class="history-separator">•</span>
                <span class="history-info">🔧 ${processing}</span>
                <span class="history-separator">•</span>
                <span class="history-info history-highlight">📦 ${formatNumber(quote.qty)}</span>
                <span class="history-separator">•</span>
                <span class="history-info history-highlight history-price">🔥 ${formatNumber(quote.sellPrice)}đ/sp</span>
                <span class="history-separator">•</span>
                <span class="history-info history-total">💰 ${formatNumber(quote.totalSell)}đ</span>
            </div>
        </div>
    `;
}
