...
function populateCatalogueDropdowns() {
    ...
    if (binding) binding.innerHTML = CATALOGUE_SETTINGS.bindings.map(b => `<option value="${b.id}">${b.name}</option>`).join('');
    if (customerType) customerType.innerHTML = CATALOGUE_SETTINGS.customerTypes.map(c => `<option value="${c.id}">${c.name}</option>`).join('');
    ...
    // Render danh sách giấy bìa và ruột
    renderCatalogueCoverPapers();
    renderCatalogueInnerPapers();
...
// ===== PREVIEW UPDATES =====
// Tự động tính margin dựa trên các công đoạn gia công
// Chỉ tự động set khi giá trị hiện tại là mặc định (5 hoặc 15), không ghi đè giá trị người dùng đã nhập
function updatePaperMargin() {
    const selectedProcs = Array.from(document.querySelectorAll('input[name="paperProc"]:checked'))
        .map(cb => parseInt(cb.value));

    // Kiểm tra xem có công đoạn bế nào không
    const hasBe = selectedProcs.some(procId => {
        const proc = PAPER_SETTINGS.processing.find(p => p.id === procId);
        return proc && (proc.name.includes('Bế demi') || proc.name.includes('Bế đứt') || proc.name.includes('Bế + Cấn'));
    });

    // Tự động set margin - chỉ khi giá trị hiện tại là mặc định (5 hoặc 15) hoặc rỗng/0
    ...
    if (marginH) {
        ...
        if (isNaN(currentValueH) || currentValueH === 0 || currentValueH === 5 || currentValueH === 15) {
            marginH.value = hasBe ? 15 : 5; // Có bế: 15mm, không có bế: 5mm
        }
    }

    if (marginV) {
        const currentValueV = parseFloat(marginV.value);
        if (isNaN(currentValueV) || currentValueV === 0 || currentValueV === 5 || currentValueV === 15) {
            marginV.value = hasBe ? 15 : 5; // Có bế: 15mm, không có bế: 5mm
        }
    }
}
